...
\begin{itemize}
	\item P11 — ranking por vía o punto crítico, por ausencia de registros en tablas \texttt{via} y \texttt{punto\_critico}.
	\item G04 — análisis por buffer de punto crítico, mientras la tabla de puntos críticos permanezca vacía.
	\item G05 — filtro por bounding box en mapa, considerado redundante frente a filtros por comuna y barrio.
\end{itemize}
...
\begin{itemize}
	\item Modelos jerárquicos o bayesianos para series territoriales con pocos datos por barrio, mitigando la limitación actual en que 220 de 270 barrios carecen de proyección individual en §3.
	\item P15 — aprendizaje automático espacial (por ejemplo, gradient boosting o modelos de conteo espacio-temporales), evaluados siempre con hold-out y sin sustituir la trazabilidad de las formulaciones actuales.
	\item Variables exógenas (meteorología, intervenciones viales, cambios normativos) para acercar las proyecciones a escenarios contrafactuales, sin pretender inferencia causal plena desde el registro administrativo.
	\item Reevaluación periódica del ranking de modelos al incorporar nuevos meses, automatizando los scripts \texttt{llenar\_evaluacion\_seccion*.py} en un pipeline de CI.
\end{itemize}
...
\begin{itemize}
	\item Generación de PDF en servidor (Puppeteer, WeasyPrint u equivalente) como alternativa a la impresión nativa del navegador.
	\item Activación del rol autoridad reservado en el esquema de permisos, con flujos de aprobación o publicación de reportes oficiales.
	\item Integración con tableros institucionales o APIs de movilidad urbana, si la entidad dispone de canales de intercambio seguros.
	\item Pruebas de carga y despliegue productivo (contenedorización, respaldos, monitoreo) que exceden el alcance de validación con \texttt{pytest} del trabajo de grado.
\end{itemize}
