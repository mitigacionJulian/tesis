...
El modelo relacional de ViaData — Medellín organiza el dominio de accidentalidad en tablas de \textbf{hechos} (\texttt{incidente}, \texttt{victima}), \textbf{catálogos} descriptivos (sexo, grupo de edad, condición, gravedad, clase de incidente), \textbf{territorio} (comuna, barrio) y tablas \textbf{opcionales} para infraestructura vial (\texttt{via}, \texttt{punto\_critico}), previstas en el esquema pero sin población desde Mede en la versión evaluada. La Figura~\ref{fig:esquema-bd} resume las relaciones principales; el DDL completo se incluye en el Anexo~B.
...
\textbf{Principio de diseño:} toda regla de limpieza reside en \texttt{mede\_limpieza.py}; EDA, pipeline guiado y memoria de grado deben importar la misma lógica para garantizar reproducibilidad.
